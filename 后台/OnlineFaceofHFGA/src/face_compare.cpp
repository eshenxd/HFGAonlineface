...
double face_compare(Main& job,People& people)
{
	...
	//vector<int>index_vec;//用来存放粗检索的结果
	//typedef pair<int, int> Psim;
	//vector<Psim>sim_vec;

	//for(unsigned int ix = 0;ix<data.size();ix++)
	//{
	//	int sim = cala_Similarity(feature,data[ix].feature_c);
	//	Psim a(ix,sim);
	//	sim_vec.push_back(a);
	//}
	

	//int Max_sim = cala_Similarity(feature,data[0].feature_c);
	...
	{
		double dis=cala_Similarity(feature,data[ix].feature_c);
